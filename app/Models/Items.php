...
class Items extends TranslateAwareModel
{
  ...
  {
    return $this->hasMany(Extras::class, 'item_id', 'id');
  }

  public function options(): HasMany
  {
    return $this->hasMany(Options::class, 'item_id', 'id');
  }

  public function variants(): HasMany
  {
    return $this->hasMany(Variants::class, 'item_id', 'id')->whereNull('deleted_at');
  }

  public function availlablevariants(): HasMany
  {
    if ($this->qty_management . '' == '1') {
      return $this->hasMany(Variants::class, 'item_id', 'id')->whereNull('deleted_at')->where('qty', '>', 0);
    } else {
  ...
  {
    return $this->belongsToMany(Allergens::class, 'item_has_allergens', 'item_id', 'allergen_id');
  }

  public function systemvariants(): HasMany
  {
    return $this->hasMany(Variants::class, 'item_id', 'id')->where('variants.is_system', 1)->whereNull('deleted_at');
  }

  public function uservariants(): HasMany
  {
    return $this->hasMany(Variants::class, 'item_id', 'id')->where('variants.is_system', 0)->whereNull('deleted_at');
  }
