...
class Categories extends TranslateAwareModel implements Sortable
{
  ...
  {
    return $this->hasMany(Items::class, 'category_id', 'id')->orderBy('items.order_index', 'asc');
  }

  public function aitems(): HasMany
  {
    return $this->hasMany(Items::class, 'category_id', 'id')->where(['items.available' => 1])->orderBy('items.order_index', 'asc');
  }

  public function restorant(): BelongsTo
  {
    return $this->belongsTo(Restorant::class, 'company_id', 'id');
  }
}