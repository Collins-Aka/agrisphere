...
class Order extends Model
{
  ...
  {
    return $this->hasOne(User::class, 'id', 'client_id')->withTrashed();
  }

  public function status(): BelongsToMany
  {
    return $this->belongsToMany(Status::class, 'order_has_status', 'order_id', 'status_id')->withPivot('user_id', 'created_at', 'comment')->orderBy('order_has_status.id', 'ASC');
  }

  public function laststatus(): BelongsToMany
  {
    return $this->belongsToMany(Status::class, 'order_has_status', 'order_id', 'status_id')->withPivot('user_id', 'created_at', 'comment')->orderBy('order_has_status.id', 'DESC')->limit(1);
  }

  public function laststatususer()
  {
    $status = $this->belongsToMany(Status::class, 'order_has_status', 'order_id', 'status_id')->withPivot('user_id', 'created_at', 'comment')->orderBy('order_has_status.id', 'DESC')->limit(1)->get();
    if (count($status) > 0) {
      try {
  ...
  {
    return $this->belongsToMany(Status::class, 'order_has_status', 'order_id', 'status_id')->withPivot('user_id', 'created_at', 'comment')->orderBy('order_has_status.id', 'DESC')->limit(1)->get();
  }

  public function getIsPreparedAttribute()
  {
    return $this->belongsToMany(Status::class, 'order_has_status', 'order_id', 'status_id')->where('status_id', 5)->count() == 1;
  }
  ...
  {
    return $this->belongsToMany(User::class, 'order_has_status', 'order_id', 'user_id')->withPivot('status_id', 'created_at', 'comment')->orderBy('order_has_status.id', 'ASC');
  }

  public function items(): BelongsToMany
  {
    return $this->belongsToMany(Items::class, 'order_has_items', 'order_id', 'item_id')->withPivot(['qty', 'extras', 'vat', 'vatvalue', 'variant_price', 'variant_name', 'id', 'kds_finished'])->withTrashed();
  }

  public function ratings(): BelongsToMany
  {
    return $this->belongsToMany(Ratings::class, 'ratings', 'order_id', 'id');
  }
  ...
  public static function boot()
  {
    ...
    self::saved(function (self $order) {
      if (strlen($order->id_per_vendor) == 0) {
        $ordersCount = Order::where('restorant_id', $order->restorant_id)->count();
  ...
  private function getOwnerOrderActions()
  {
    ...
    if (config('app.isft')) {
      ...
      if (in_array($lastStatusAlias, ['just_created', 'updated'])) {
        ...
        if (FacadesModule::has('drivers') && $this->delivery_method . '' != '3') {
          return ['buttons' => ['delivered', 'assigned_to_driver'], 'message' => ''];
        } else {
          //No Drivers
          return ['buttons' => ['delivered'], 'message' => ''];
